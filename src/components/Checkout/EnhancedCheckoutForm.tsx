...
export function EnhancedCheckoutForm() {
  ...
  const handleStripePaymentSuccess = async (paymentMethodId: string) => {
    if (!paymentState.clientSecret) {
      setFormStatus({
        submitting: false,
        success: false,
        error: 'Greška u obradi plaćanja. Molimo pokušajte ponovno.'
      });
      return;
    }

    setPaymentState(prev => ({ ...prev, processingPayment: true }));
    ...
    try {
      ...
      const confirmResult = await confirmPayment(orderData);

             if (confirmResult.success) {
         // Store the order in context (adapt to expected format)
         const orderResponse = {
           success: true,
           orderId: confirmResult.orderId || '',
           message: confirmResult.message || 'Order placed successfully'
         };
      ...
      setFormStatus({
        submitting: false,
        success: false,
        error: error instanceof Error ? error.message : 'Došlo je do problema s vašom narudžbom.'
      });
    } finally {
      setPaymentState(prev => ({ ...prev, processingPayment: false }));
    }
  };
  ...
  // Show Stripe payment form if payment method is credit card and we have client secret
  if (paymentState.showStripeForm && paymentState.clientSecret) {
    return (
      <div className="checkout-form-container">
        <div className="payment-step-header">
          <h2>Platite karticom</h2>
          <p>Ukupno za plaćanje: <strong>€{getTotalPrice().toFixed(2)}</strong></p>
        </div>
        ...
        <div className="payment-actions">
          <button
            type="button"
            onClick={() => setPaymentState(prev => ({ ...prev, showStripeForm: false }))}
